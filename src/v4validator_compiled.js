...
V4Validator.prototype = {
    ...
    errorIf: function (condition) {
        if (condition) {
            this.error.apply(this, [].slice.call(arguments, 1));
        }
    },
    ...
    "[^required]": {prepare: function (s, ctx) {
        ...
        return {inline: "if (_ === undefined) ctx.stop()"};    //TODO: ???
    }},
    "[type=string]": {inline: function (_, ctx) {
        this.errorIf(typeof _ !== 'string', 'type.string', ctx);
    }},
    "[type=number]": {inline: function (_, ctx) {
        this.errorIf(typeof _ !== 'number', 'type.number', ctx);
    }},
    "[type=integer]": {inline: function (_, ctx) {
        this.errorIf((typeof _ !== 'number') || (_ % 1 !== 0), 'type.integer', ctx);
    }},
    "[type=null]": {inline: function (_, ctx) {
        this.errorIf(_ !== null, 'type.null', ctx);
    }},
    "[type=boolean]": {inline: function (_, ctx) {
        this.errorIf(typeof _ !== 'boolean', 'type.boolean', ctx);
    }},
    "[type=array]": {inline: function (_, ctx) {
        this.errorIf(!Array.isArray(_), 'type.array', ctx);
    }},
    "[type=object]": {inline: function (_, ctx) {
        this.errorIf(Array.isArray(_) || typeof _ !== 'object' || _ === null, 'type.object', ctx);
    }},
    ...
    "[enum]": function (schema) {
        ...
        return {inline: "this.errorIf(!this.$enums[" + (this.$enums.length-1) + "][this.toComparable(_)], 'type.enum', ctx, " + JSON.stringify(schema.enum) + ")"};
    },

    //////////////// string

    "[maxLength]": function (schema) {
        return {inline: "if (typeof _ === 'string') this.errorIf(_.length > " + schema.maxLength + ", 'type.string.maxLength', ctx, " + schema.maxLength + ")"}
    },
    "[minLength]": function (schema) {
        return {inline: "if (typeof _ === 'string') this.errorIf(_.length < " + schema.minLength + ", 'type.string.minLength', ctx, " + schema.minLength + ")"}
    },
    "[pattern]": function (schema) {
        return {inline: "if (typeof _ === 'string') this.errorIf(!_.match(/" + schema.pattern + "/), 'type.string.pattern', ctx, " + JSON.stringify(schema.pattern) + ")"}
    },
    "[format]": function (schema) {
        ...
        return {inline: "if (typeof _ === 'string') this.errorIf(!_.match(" + fmt.regexp + "), 'type.string.format." + schema.format + "', ctx, " + JSON.stringify(fmt.message) + ")"}
    },
    ...
    "[minItems]": function (schema) {
        return {
            inline: "this.errorIf(Array.isArray(_) && _.length < " + schema.minItems + ", 'type.array.minItems', ctx)"
        }
    },

    "[maxItems]": function (schema) {
        return {
            inline: "this.errorIf(Array.isArray(_) && _.length > " + schema.maxItems + ", 'type.array.maxItems', ctx)"
        }
    },
    ...
    "[additionalProperty=false]": {inline: function (_, ctx) {
       // if (typeof _ !== 'object' || Array.isArray(_)) return;
